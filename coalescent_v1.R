...
n_tips <- 10 #number of tips in species trees and gene trees
n_gene_trees <- 5 #number of gene trees to simulate for each species tree
locus_size <- 500 #length in bp for each locus simulated along each gene tree
n_reps <- 5 #total number of replicate experiments - number of species trees
effective_population_size_approx <- 1000 #currently just an appoximation
...
print(to_join[[1]]$tip.label)
print(to_join[[2]]$tip.label)
}
}
}
}
...
dir.create(paste(z, "gene_sequences", sep=""))
for (i in 1:length(file_names)){
write.phyDat(sequences[[i]], file = paste(z, paste("gene_sequences/", file_names[[i]], sep=""), sep=""), format = "nexus")
}
...
dir.create(paste(z, "concat_sequence", sep=""))
write.phyDat(concat, file = paste(z, paste("concat_sequence/", "concat.nexus", sep=""), sep=""), format = "nexus")

#########

dir.create(paste(z, "species_tree", sep=""))
write.tree(entire_tree, file = paste(z, paste("species_tree/", "species_tree.tre", sep=""), sep=""))
...
dir.create(paste(z, "clade_support", sep=""))
sink(paste(z, "clade_support/clade_support.txt", sep=""))
